        ...
        foreach ($logs as $event) {
            ...
            switch ($event['type']) {
                case 'startPeriod':
                    ...
                    if (count($players)) {
                        $this->goToPlay($match->getHomeTeam(), $players, $finishPrevPeriod);
                    }
                    $players = $details['team2']['startPlayerNumbers'] ?? [];
                    if (count($players)) {
                        $this->goToPlay($match->getAwayTeam(), $players, $finishPrevPeriod);
                    }
                    break;
                case 'finishPeriod':
                    if ($period === 2) {
                ...
                case 'goal':
                    $team = $this->getTeamByName($match, $details['team']);
                    $team->addGoal();
                    $team->getPlayer($details['playerNumber'])->makeGoal();
                    break;
                case 'yellowCard':
                    $team = $this->getTeamByName($match, $details['team']);
                    $team->getPlayer($details['playerNumber'])->makeYellow();
                    break;
                case 'redCard':
                    $team = $this->getTeamByName($match, $details['team']);
